...
const App = () => {
  ...
  const handleGenerateText = useCallback(async () => {
    ...
    try {
      const payload = { model: 'gemini-2.5-flash', contents: prompt, ...(prepaidCodeState.isValid && { prepaidCode: prepaidCodeState.code }) };
  ...
  const handleCheckOriginality = useCallback(async () => {
    ...
    try {
      const response = await callApiViaProxy({ model: 'gemini-2.5-flash', contents: prompt, config: { responseMimeType: 'application/json' } });
  ...
  const handleIncreaseUniqueness = useCallback(async () => {
    ...
    try {
      const response = await callApiViaProxy({ model: 'gemini-2.5-flash', contents: prompt });
  ...
  const handleGetSolution = useCallback(async () => {
    ...
    try {
        ...
        const payload = { model: 'gemini-2.5-flash', contents, ...(prepaidCodeState.isValid && { prepaidCode: prepaidCodeState.code }) };
        const response = await callApiViaProxy(payload);
